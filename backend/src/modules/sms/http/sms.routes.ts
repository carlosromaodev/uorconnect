...
function extractCredits(payload: unknown): number | null {
  ...
  const record = payload as Record<string, unknown>;
  const possible = [record.balance, record.credits, record.available, record.saldo];
    ...
    protectedApp.register(async (adminApp) => {
      ...
      adminApp.get("/admin/overview", {
        ...
        if (ombala.isConfigured) {
          ...
          providerStatus = sendersResponse.ok || creditsResponse.ok ? "ok" : "error";

          if (!sendersResponse.ok && !creditsResponse.ok) {
            providerMessage = pickString((sendersResponse.payload as Record<string, unknown>)?.message)
              ?? pickString((creditsResponse.payload as Record<string, unknown>)?.message)
      ...
      adminApp.get("/admin/filters", {
        ...
        const studentCourseButtons = await buildStudentCourseButtons();
