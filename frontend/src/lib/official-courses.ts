export const OFFICIAL_COURSE_OPTIONS = [
  "Direito",
...
const OFFICIAL_COURSE_SET = new Set<string>(OFFICIAL_COURSE_OPTIONS);
...
export function getOfficialCourseFieldValue(value?: string | null) {
  return normalizeOfficialCourse(value) ?? value?.trim() ?? "";
}

export function getOfficialCourseSelectOptions(value?: string | null) {
  const currentValue = getOfficialCourseFieldValue(value);

  if (!currentValue || OFFICIAL_COURSE_SET.has(currentValue)) {
    return [...OFFICIAL_COURSE_OPTIONS];
  }

  return [currentValue, ...OFFICIAL_COURSE_OPTIONS];
}
