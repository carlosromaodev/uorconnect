...
export function FeaturedCourseCard({
  ...
  return (
    <article
      className={cn(
        "relative h-full overflow-hidden rounded-2xl border bg-card p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl sm:p-5 md:p-6",
        className
      )}
      style={{
      ...
      <div className="relative z-10 flex h-full min-w-0 flex-col">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-4">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white/80 shadow-sm ring-1 ring-white/60">
              {course.companyLogoUrl ? (
                <img src={course.companyLogoUrl} alt={course.companyName} className="h-9 w-9 rounded-xl object-cover" />
              ) : (
                <BookOpen className="h-5 w-5" style={{ color: course.courseColor }} />
              )}
            </div>
            <div className="min-w-0">
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] md:text-xs" style={{ color: course.courseColor }}>
                {course.isPaid ? course.priceLabel || "Pago" : "Gratuito"}
              </p>
              <h3 className="safe-break line-clamp-2 font-heading text-lg font-bold leading-tight md:text-xl">{course.name}</h3>
              <p className="mt-1 truncate text-xs font-medium text-foreground/80 md:text-sm">{course.companyName}</p>
            </div>
          </div>
          <span
        ...
        <p className="safe-break line-clamp-3 text-sm leading-6 text-foreground/90 md:text-base">{course.description}</p>
        {course.preview ? (
          <p className="safe-break mt-2 line-clamp-2 text-sm leading-6 text-muted-foreground">{course.preview}</p>
        ) : null}

        <div className="mt-4 rounded-xl border border-white/60 bg-white/72 p-3.5">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-sm font-semibold text-foreground md:text-base">{course.companyName}</p>
              <p className="truncate text-xs font-medium text-muted-foreground md:text-sm">{course.companyCategory}</p>
            </div>
          </div>
          <div className="mt-3 flex flex-wrap gap-3 text-xs font-medium text-muted-foreground md:text-sm">
        ...
        <div className="mt-4 grid gap-2.5 sm:mt-auto sm:grid-cols-2">
          <Button
            size="sm"
