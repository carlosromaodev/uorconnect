...
export function FeaturedCourseCard({
  ...
  const helperMessage = enrollBlocked
    ...
    : "Depois da inscrição, o recibo e os detalhes ficam disponíveis aqui.";

  const hasSocialLinks =
    course.companyWebsite || course.companyInstagram || course.companyLinkedin;
  const companyInitial = course.companyName.trim().charAt(0).toUpperCase() || "E";
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: `linear-gradient(180deg, transparent, ${withAlpha(course.courseColor, "55")}, transparent)` }}
      />
      <div
        className="pointer-events-none absolute right-0 top-0 h-28 w-28 opacity-[0.09]"
        style={{
          backgroundImage: `linear-gradient(135deg, ${course.courseColor} 1px, transparent 1px)`,
      ...
      <div
        ...
        style={{
          background: `linear-gradient(145deg, ${withAlpha(course.courseColor, "10")} 0%, ${withAlpha(course.accentColorSecondary, "08")} 58%, transparent 100%)`,
        }}
      >
        <div className="relative flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div
              className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border shadow-sm sm:h-14 sm:w-14"
              style={{
        ...
        <div className="relative mt-4 grid grid-cols-2 gap-2">
          <div className="rounded-xl border border-white/80 bg-white/75 px-3 py-2 shadow-sm">
            <p className="text-base font-bold leading-none" style={{ color: course.courseColor }}>
              {course.studentCount}
            </p>
            <p className="mt-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">inscritos</p>
          </div>
          <div className="rounded-xl border border-white/80 bg-white/75 px-3 py-2 shadow-sm">
            <p className="text-base font-bold leading-none" style={{ color: course.courseColor }}>
              {course.likesCount}
            </p>
            <p className="mt-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">curtidas</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            >
              {course.companyLogoUrl ? (
                ...
                <span style={{ color: course.courseColor }}>{companyInitial}</span>
              )}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 truncate text-sm font-semibold text-foreground">{course.companyName}</p>
              <p className="truncate text-xs text-muted-foreground">{course.companyCategory}</p>
            </div>
            {enrollmentStatusLabel ? (
              <span
                className="inline-flex shrink-0 items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-semibold sm:text-[11px]"
                style={{
                  borderColor: withAlpha(course.courseColor, "25"),
