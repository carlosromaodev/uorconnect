...
import { AlertTriangle, LayoutDashboard, Loader2, Radio, Shield, Trophy } from "lucide-react";
...
export function LaboratorioAdminShell({
        ...
        <div className="mx-auto max-w-3xl">
          <ContestCard className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#00e5c8]/16 bg-[#00e5c8]/10 text-[#00e5c8]">
              ...
              <p className="text-sm leading-7 text-[#8ea1b8]">
                O painel administrativo do Laboratório depende de autenticação académica e autorização explícita para gestão da arena.
              </p>
            </div>
  ...
  return (
    <div className={cn("min-h-screen", contestTheme.shell)}>
      ...
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_top_left,rgba(0,229,200,0.14),transparent_24%),radial-gradient(circle_at_92%_10%,rgba(22,249,254,0.12),transparent_18%)]" />

      <div className="relative z-10 flex min-h-screen">
        <aside className="hidden w-[280px] border-r border-white/8 bg-[rgba(7,14,18,0.96)] px-5 py-6 lg:block">
          <p className="font-tech-mono text-[10px] uppercase tracking-[0.22em] text-[#00e5c8]">laboratorio.admin</p>
          ...
          <p className="mt-3 text-sm leading-7 text-[#8ea1b8]">
            Gestão da arena, segurança e leitura operacional da sessão do Laboratório.
          </p>
            ...
            {adminItems.map((item) => (
              <NavLink
                ...
                className={({ isActive }) => cn(
                  ...
                  isActive
                    ? "border-[#00e5c8]/24 bg-[#00e5c8]/10 text-[#00e5c8]"
                    : "border-white/8 bg-white/[0.03] text-[#94a6bb] hover:border-[#00e5c8]/16 hover:text-white",
                )}
              >
          ...
          <ContestCard className="mt-8">
            <p className="font-tech-mono text-[10px] uppercase tracking-[0.18em] text-[#7b8ca3]">{clock.label}</p>
            <p className="mt-2 text-xl font-semibold text-white">{clock.display}</p>
            <p className="mt-2 text-sm text-[#8ea1b8]">{contestConfig.course}</p>
          </ContestCard>
        </aside>

        <div className="flex-1 px-4 py-6 md:px-6">
          <header className="mb-6 flex flex-col gap-4 rounded-[28px] border border-white/8 bg-[rgba(8,16,20,0.86)] px-5 py-4 backdrop-blur-xl md:flex-row md:items-center md:justify-between">
            <div>
              <p className="font-tech-mono text-[10px] uppercase tracking-[0.18em] text-[#00e5c8]">admin.runtime</p>
              <h2 className="mt-2 text-2xl font-semibold text-white">Operação da arena</h2>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Button asChild className={cn("h-10 px-4", contestButtonClassNames.primary)}>
                <Link to="/arena">Abrir arena</Link>
              </Button>
            </div>
          </header>
