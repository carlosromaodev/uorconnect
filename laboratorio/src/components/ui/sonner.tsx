...
const notifyDefaults: Record<NotifyKind, { duration: number; type: TypeOptions; icon: ReactNode }> = {
  success: { duration: 4200, type: "success", icon: <CheckCircle2 className="h-4 w-4" /> },
  error: { duration: 6500, type: "error", icon: <XCircle className="h-4 w-4" /> },
  warning: { duration: 7000, type: "warning", icon: <TriangleAlert className="h-4 w-4" /> },
  info: { duration: 5200, type: "info", icon: <Info className="h-4 w-4" /> },
  neutral: { duration: 5200, type: "default", icon: <Info className="h-4 w-4" /> },
};

function ToastBody({ title, description }: { title: ReactNode; description?: ReactNode }) {
  return (
...
function createToast(kind: NotifyKind, title: ReactNode, options?: NotifyOptions) {
  const defaults = notifyDefaults[kind];
  const content: ToastContent = <ToastBody title={title} description={options?.description} />;
